...
PATH = "/home/teamspeak/Teamspeak/chromedriver"
...
class TsBot:
    def __init__(self, **kwargs):
        ...
        self.ts3conn.exec_("clientmove", clid=resp.parsed[0]["client_id"], cid=20)
        
    def updateGuildMembersInDatabase(self):

        enemyguildname = ["The Round Table"]
        for enemieGuild in enemyguildname:
            ...
            enemyGuilds = f"https://kingdom-swap.com/guilds.php?name={enemyGuild}"
        ...
        enemyguildmakersname = ["Amiguinhos Academy"]
        ...
        friendguildname = ["Red Ribbon Army", "Total War", "Latin Kings PVP", "Peaky Blinders CO", "Unidos Contra Power Abuse"]
        ...
        Hunteds = ["Sharp Bolt", "Forrozin", "Shantos", "Firs Sativa", "Sir Petreca",
                      ...
                      "Miyamoto Musashi", "Pedro Luz", "Babydoge Tothemoon",
                       "Greijal", "Malefian Death",
                        "Like", "Schwarzenegger", "Servetti",
                         "Jorginho The Archer", "Pala Rush", "Coronel Budapeste", "Ozzy",
        ...
        for hunted in Hunteds:
            ...
            if cursor.fetchone() is None:
                ...
                try:
                    ...
                    huntedlevel = huntedcharacter.find_element_by_xpath((f"//div[@id='content']/ul/li[2]"))
                    huntedvocation = huntedcharacter.find_element_by_xpath((f"//div[@id='content']/ul/li[3]"))
    ...
    def comandos(self):
        ...
        try:
            self.ts3conn.exec_("servernotifyregister", event="textprivate")

            recebido = self.ts3conn._recv()
            print("comando Vermelho")
            ...
            if comando[0] == "!mp":
                ...
                for client in self.ts3conn.exec_("clientlist"):
                    ...
                    if client['client_type'] != '1':
                        ...
                        maspoke = " ".join(poke)
                        self.ts3conn.exec_("clientpoke", clid=client["clid"], msg=f"{recebido.parsed[0]['invokername']}: {maspoke}")  
                    ...
                    if client['client_type'] != '1' and client['client_type'] != '0':
                        ...
                        if len(comando) > 1:
                            ...
                            for i in range(1, len(comando), 1):
                                poke.append(comando[i])
                            maspoke = " ".join(poke)
                            self.ts3conn.exec_("clientkick", clid=client["clid"], reasonid=5, reasonmsg=f"{recebido.parsed[0]['invokername']}: {maspoke}")
                        else:
