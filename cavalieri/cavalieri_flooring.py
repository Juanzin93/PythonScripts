...
# usefull functions
    ...
    def productUpdate(self, data):  
        ...
        for product in data:
            ...
            self.product_tree.insert("", "end", values=(product['Product'], product['Size'], product['Thickness'], product['Wear Layer'], product['Finish'], product['Brand'], product['SKU'], product['Sqft/Box'], product['Pc/Box'], product['Box/Pallet'], product['Lbs/Box'], product['Dealer Price'], product['Retail Price'], product['Box_In_Stock'], product['Material'], product['Color Group']))
...
#buttons
    ...
    def addProduct(self):
        if self.addProductWindow is None or not self.addProductWindow.winfo_exists():
            ...
            self.addProductWindow.geometry("400x450")
            self.addProductWindow.iconbitmap('property_manager_tool2.ico')
            self.addProductWindow.minsize(400, 450)
            self.addProductWindow.maxsize(400, 450)
            ...
            self.add_button.place(x=115,y=340)
    ...
    def productList(self):
        if self.productListWindow is None or not self.productListWindow.winfo_exists():
            ...
            self.product_tree['columns'] = ('Product', 'Size', 'Thickness', 'Wear Layer', 'Finish', 'Brand', 'SKU', 'Sqft/Box', 'Pc/Box', 'Box/Pallet', 'Lbs/Box', 'Dealer Price', 'Retail Price', 'Box In Stock', 'Material', 'Color Group')
            ...
            #self.reloadProducts(self.searchMaterial.get().lower())
    ...
    def reloadProducts(self, material):
        for deleteProduct in self.product_tree.get_children():
            self.product_tree.delete(deleteProduct)

        inventory_size = self.getSocketDataInventorySize(f"getSizeInventory,m,{material}")
        for i in range(int(inventory_size)):
            data = self.getSocketData(f"getInventory,{i},{material}") #[
            if not str(data) == 'denied':
                db_data = json.loads(data)
                self.product_tree.insert("", "end", values=(db_data['Product'], db_data['Size'], db_data['Thickness'], db_data['Wear Layer'], db_data['Finish'], db_data['SKU'], db_data['Sqft/Box'], db_data['Pc/Box'], db_data['Box/Pallet'], db_data['Lbs/Box'], db_data['Dealer Price'], db_data['Retail Price'], db_data['Box_In_Stock'], db_data['Material']))

    def OpenSelectedproduct(self, product):
        self.productSelected = customtkinter.CTkToplevel(self)
        self.productSelected.title(self.product_tree.item(self.product_tree.selection())["values"][1])
        self.productSelected.geometry(f"{App.WIDTH+400}x{App.HEIGHT+150}")
        self.productSelected.iconbitmap('property_manager_tool2.ico')
        self.productSelected.minsize(App.WIDTH+400, App.HEIGHT+150)
        self.productSelected.columnconfigure(2, weight=1)
        self.productSelected.rowconfigure(2, weight=1)
        self.productSelected_frame = customtkinter.CTkFrame(master=self.productSelected)
        self.productSelected_frame.grid(row=1, column=1, rowspan=2, columnspan=2, sticky="nswe", padx=20, pady=50)
        
        self.productSelected_frame.columnconfigure(1, weight=1)
        self.productSelected_frame.rowconfigure(12, weight=1)
