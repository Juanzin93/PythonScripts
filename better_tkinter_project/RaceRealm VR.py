...
class App(customtkinter.CTk):
    ...
    def __init__(self):
        ...
        self.frame_dashboard.columnconfigure((0, 1), weight=1)
        self.frame_dashboard.columnconfigure(2, weight=0)
        
        
        self.currently_playing = customtkinter.CTkScrollableFrame(self.frame_dashboard)
        self.currently_playing.grid(row=0, column=0, columnspan=1, rowspan=8, pady=20, padx=20, sticky="nwse")
        
        self.highscores_frame = customtkinter.CTkScrollableFrame(self.frame_dashboard)
        self.highscores_frame.grid(row=0, column=1, columnspan=2, rowspan=8, pady=20, padx=20, sticky="nwse")

        # configure grid layout (1x1)
        self.currently_playing.rowconfigure(0, weight=1)
        self.currently_playing.columnconfigure(0, weight=1)
        
        self.highscores_frame.rowconfigure(0, weight=1)
        self.highscores_frame.columnconfigure(0, weight=1)
        
        self.UsersPlayingLabel_1 = customtkinter.CTkLabel(master=self.currently_playing,
                                              text="Playing Now",
                                              )  # font name and size in px
        self.UsersPlayingLabel_1.grid(row=0, column=0, pady=1, padx=10)

        self.UsersPlayingLabel_2 = customtkinter.CTkLabel(master=self.highscores_frame,
                                              text="Ranking",
                                              )  # font name and size in px
        self.UsersPlayingLabel_2.grid(row=0, column=0, pady=1, padx=10)

        temp_user = "Juan"
        temp_seat = "1"
        temp_start_time = "20:00"
        temp_end_time = "21:00"
        
        for i in range(10):
            seat = i+1
            self.label_user = customtkinter.CTkLabel(master=self.currently_playing,
                                                    text=f"User: {temp_user} \n" +
                                                    f"Seat: {seat}   Start Time: {temp_start_time}\n" +
                                                    f"End Time: {temp_end_time}",
                                                    height=10,
